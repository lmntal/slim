...
#define max_insert_length max_lazy_match
  ...
  struct ct_data_s dyn_ztree[2 * ZD_CODES + 1]; /* zdelta: extra flag           */
 ...
 */
void zd_tr_flush_block OF((deflate_state * s, charf *buf, ulg stored_len, int eof));
void zd_tr_align OF((deflate_state * s));
void zd_tr_stored_block OF((deflate_state * s, charf *buf, ulg stored_len, int eof));

#define d_code(dist) ((dist) < 256 ? zd_dist_code[dist] : zd_dist_code[256 + ((dist) >> 7)])

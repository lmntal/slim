...
OmegaArray *makeOmegaArray() {
  ...
  ret->body = new unbound_vector<IntContainer *>();

  return ret;
}

void freeOmegaArray(OmegaArray *oArray) {
  for (auto v : *oArray->body)
    free(v);
  oArray->body->clear();
  delete oArray->body;
  free(oArray);
...
int readOmegaArray(OmegaArray *oArray, int externalIndex) {
  ...
  IntContainer *tmp = (IntContainer *)oArray->body->read(internalIndex);
...
/*
   ...
   IntContainer *tmp = oArray->body->read(internalIndex);
...
void incrementOmegaArray(OmegaArray *oArray, int externalIndex) {
  if (externalIndex >= 0) {
    ...
    if (tmp == NULL) {
      tmp = makeInt();
      oArray->body->write(internalIndex, tmp);
      tmp->value++;
    } else {
      tmp->value++;
    }
...
void decrementOmegaArray(OmegaArray *oArray, int externalIndex) {
  if (externalIndex >= 0) {
    ...
    if (externalIndex == oArray->maxFiniteIndex) {
      while (readOmegaArray(oArray, oArray->maxFiniteIndex) == 0 &&
             oArray->maxFiniteIndex >= 0) {

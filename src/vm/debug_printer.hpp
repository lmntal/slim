 ...
 * 桁数は最小限になる
 */
template <class T> inline std::string to_hex_string(T value) {
  if (value == 0) {
    return "0x0";
  }
 ...
 * 桁数は最小限になる
 */
template <> inline std::string to_hex_string<unsigned char>(unsigned char value) {
 ...
 */
inline std::string print_object_ids(void const *address, unsigned long id) {

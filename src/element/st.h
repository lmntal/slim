...
using st_cmp_func  = int (*)(void *, void *);
using st_hash_func = long (*)(void *);
using st_iter_func = int (*)(...);

struct st_hash_type {
  st_cmp_func
      compare; // 対象の2つのエントリー(st_table_entry)が同じチェインに属するならば偽、そうでなければ真を返す関数
  st_hash_func hash; // ハッシュ関数
};
...
static inline unsigned long st_num(st_table_t table) { return table->num_entries; }

static inline unsigned long st_cap(st_table_t table) { return table->num_bins; }
...
long          st_strhash(char const *str);
...
void st_concat(st_table_t tbl1, st_table_t tbl2);
...
public:
  ...
  st_iterator(st_iterator const &it);
  ~st_iterator() noexcept = default;

  st_iterator &operator=(st_iterator const &it) noexcept;
  ...
  bool operator!=(st_iterator const &a) { return !(*this == a); }
  bool operator==(st_iterator const &a) { return ptr == a.ptr; }
};

inline st_iterator begin(st_table_t const &table) { return (table) ? st_iterator(table) : st_iterator(); }
inline st_iterator end(st_table_t const &table) { return {}; }
